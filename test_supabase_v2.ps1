...
Write-Host "1. Creating Test Client with UUID: $TestId"
# Timestamptz expects ISO 8601 string, NOT number
$IsoDate = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")

$Body = @{
    id                  = $TestId
    nombre              = "Test PS Delete Valid UUID"
    telefono            = "000000000"
    fecha_creacion      = $IsoDate
    ultima_modificacion = $IsoDate
    user_id             = $null # Explicitly null or omit if not authenticated
} | ConvertTo-Json
...
try {
    ...
    if ($Check.Count -gt 0) {
        Write-Host "Record confirmed."
    }
    else {
        Write-Host "Record NOT found (Create might have been blocked or RLS hidden it)."
        exit
    }
}
catch {
