...
class ClientsUI {
    ...
    async render() {
        try {
            ...
            // Filter out system clients (Cliente General) from the visible list
            this.clientes = this.clientes.filter(c => {
                const isSystemId = c.id === 'CLIENTE_GENERAL';
                const isSystemName = c.nombre === 'Cliente' && c.apellido === 'General';
                return !isSystemId && !isSystemName;
            });
        ...
        return `
            <div class="card" data-id="${cliente.id}">
                ...
                <div style="flex: 1; min-width: 0; text-align: left;">
                    <h3 class="card-title" style="margin: 0 0 4px 0; font-size: 1rem; font-weight: 700; color: var(--text-primary); text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">
                        ${this.escapeHtml(cliente.nombre)} ${this.escapeHtml(cliente.apellido || '')}
                    </h3>
                    <div style="display: flex; flex-direction: column; gap: 2px; font-size: 0.9rem; color: var(--text-secondary);">
                        ...
                        <div style="display: flex; align-items: center; gap: 4px;">
                            ...
                            <span>DNI: ${this.escapeHtml(cliente.dni)}</span>
                        </div>
                        ` : ''}
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 13px; height: 13px; color: var(--text-muted); flex-shrink: 0;">
                            ...
                            <span>Tel: ${this.escapeHtml(cliente.telefono || 'N/A')}</span>
                        </div>
                        ${cliente.email ? `
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 13px; height: 13px; color: var(--text-muted); flex-shrink: 0;">
                ...
                <div style="display: flex; gap: 6px; flex-shrink: 0; align-items: center;">
                    <button class="btn btn-icon btn-sm btn-edit" data-action="edit" data-id="${cliente.id}" title="${i18n.t('btn_edit')}">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                        </svg>
                    </button>
                    <button class="btn btn-icon btn-sm btn-repairs" data-action="repairs" data-id="${cliente.id}" title="${i18n.t('btn_view')}">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path>
                        </svg>
                    </button>
                    <button class="btn btn-icon btn-sm btn-delete" data-action="delete" data-id="${cliente.id}" title="${i18n.t('btn_delete')}" style="color: var(--danger);">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    async deleteCliente(id) {
        if (id === 'CLIENTE_GENERAL') {
            app.showToast('No se puede eliminar el Cliente General', 'error');
            return;
        }
